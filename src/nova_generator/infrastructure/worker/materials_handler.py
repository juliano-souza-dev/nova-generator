...
class MaterialsJobHandler:
    ...
    def synthesize(self, job: Job, execution: JobExecution) -> dict[str, Any]:
        ...
        execution.heartbeat()
        return {"cue_id": str(cue_id), "duration_ms": speech.duration_ms,
                "audio_sha256": ExportCue(cue_id, 1, text, cue.approved_pt,
                                          Path(speech.audio_path), speech.duration_ms,
                                          utf8_sha256(text)).audio_sha256}
    ...
    def export(self, job: Job, execution: JobExecution) -> dict[str, Any]:
        ...
        for order, raw_id in enumerate(raw_ids, 1):
            ...
            cues.append(ExportCue(
                cue.id, order, cue.approved_en, cue.approved_pt,
                Path(speech.audio_path), speech.duration_ms,
        ...
        result = self._exporter.execute(
            ...
            output_directory=self._export_root / str(job.id), deck_name=project.title,
        )
        execution.heartbeat()
        return {"apkg": str(result.apkg_path), "manifest": str(result.manifest_path),
                "reel": str(result.reel_path), "cards": len(cues)}
...
def _snapshot(raw: object) -> VoiceProfileSnapshot:
    ...
    snapshot = VoiceProfileSnapshot(
        profile_id=UUID(str(raw["profile_id"])), name=str(raw["name"]),
        version=int(raw["version"]), model_id=str(raw["model_id"]),
        model_sha256=str(raw["model_sha256"]),
        reference_audio_sha256=str(raw["reference_audio_sha256"])
        if raw.get("reference_audio_sha256") else None,
